...
Самостоятельная энциклопедия моделей ИИ и инструментов для работы с ИИ.
Публичный интерфейс состоит из двух независимых каталогов: «Модели» и
«Инструменты». Существующий публичный сайт https://aipediya.com.
...
на него ссылаются и не копируют. Текущее состояние — `docs/EXECUTION_STATE.md`.
Пакет для нового чата: обновить статус и выполнить
`.\.venv\Scripts\python.exe tools/pack_ai_context.py`, затем прикрепить
...
отдельные карточки инструментов, страны происхождения, платформы, источники,
разработчики, сервисы, тарифы, проверки и дополнительные факты.
Поля текста двуязычные: `{"ru": "Текст", "en": "Text"}`.
У каждой версии модели есть встроенные таблицы доступа, цен, проверок и фактов.
Инструменты используют отдельную сущность и отдельный публичный набор полей;
сохранённая legacy-запись связывается с инструментом и не удаляется.
